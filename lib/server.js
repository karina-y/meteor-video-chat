...
const Services = {
    ...
    initializeCallSession(_id, meteorUser) {
        this.destroyOldCalls(meteorUser);
    ...
    call(_id) {
        check(_id, String);
        const meteorUser = Services.getUser(_id);
        if (Services.checkConnect(meteorUser._id, _id)) {
            ...
            if (inCall) {
                ...
                return Services.initializeCallSession(_id, meteorUser);
            }
        }
        else {
    ...
    reject() {
        ...
        if (user) {
            ...
            throw new Meteor.Error(403, "Could not find user");
        }
    }
};
